...
class Icon extends HTMLElement {
    ...
    connectedCallback() {
        this.hasAttribute("tabindex") || this.setAttribute("tabindex", 0), this.i = this.parentNode.querySelectorAll("fos-icon").length - 1, this.render()
    }
    static get observedAttributes() {
...
class Desktop extends HTMLElement {
    ...
    render() {
        this.shadow.innerHTML = "<style>:host{position: relative;display: block;width: 100%;height: 100%;min-height: 240px;min-width: 320px;background-color: #666;overflow: auto;}</style><slot></slot>"
    }
}
...
class Menu extends HTMLElement {
    ...
    connectedCallback() {
        for (this.hasAttribute("tabindex") || this.setAttribute("tabindex", 0), this.bar = this.parentNode;
            "FOS-BAR" !== this.bar.tagName;) this.bar = this.bar.parentNode;
        this.render()
    }
    static get observedAttributes() {
        return ["name", "visible"]
